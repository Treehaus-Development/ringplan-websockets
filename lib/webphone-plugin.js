...
var dialpadSounds = ["dial-1.wav", "dial-2.wav", "dial-3.wav"];
let contactsValues = [];
...
(function ($) {
  ...
  function _fSansayUiKeypadInit() {
    ...
    if (isFromSSO) {
      if (sessionStorage.getItem("contacts")) {
        contactsValues = JSON.parse(sessionStorage.getItem("contacts"));
      } else {
        getContacts(true).then((data) => {
          contactsValues = [...data];
        });
      }
    }
    ...
    $("#webphone-0-btn").click(function () {
      _fSansayUiRegularKey(longPress ? "+" : "0");
    });
    $("#webphone-pound-btn").click(function () {
      _fSansayUiRegularKey("#");
    });

    $("#webphone-0-btn").on("mousedown", function () {
      startTime = new Date().getTime();
    });

    $("#webphone-0-btn").on("mouseup", function () {
    ...
    $("body").on("click", function (e) {
      if (!$("#call-active").hasClass("hidden")) {
        if (e.target.id !== "speaker-img" && e.target.id !== "volume") {
    ...
    function handleInput(e) {
      ...
      if (contactsValues.length > 0) {
        ...
        timeout = setTimeout(function () {
          let searchResults = document.getElementById("filter-results");
          const cleanedString = e.target.value.replace(/[\+\*\#]/g, "");
          const newData = filterContacts(contactsValues, cleanedString, true);
          const finalData = newData.slice(0, 100);
          const htmlData = template(finalData, true);
          searchResults.innerHTML = htmlData;
          $("#filter-results-wrapper").removeClass("hidden");
          if (finalData.length === 0) {
            $("#filter-results-wrapper").addClass("hidden");
            $("#add-new-contact").removeClass("hidden");
            $("#add-new-contact").addClass("flex");
            $("#add-new-contact").click(function (e) {
              document.getElementById("contacts-tab").props = {
                isAdd: true,
                value: inputVal,
              };
            ...
            $("#add-new-contact").removeClass("flex");
            $("#add-new-contact").addClass("hidden");
          }
        }, 500);
      }
    }
    ...
    document
      ...
      .addEventListener("keydown", function (event) {
        ...
        if (
          !(key >= 48 && key <= 57) &&
          ![8, 9, 13, 37, 38, 39, 40].includes(key) &&
          ![107, 106, 35].includes(key) &&
          event.key !== "v"
        ) {
  ...
  function _fSansayUiBackspace() {
    ...
    if (cur_digits.length > 0)
      $(".webphone-digits").val(cur_digits.slice(0, -1));
    else $(".webphone-digits").val("");
  ...
  function _fSansayUiCallKey(video_call) {
    ...
    if (_vSansayUiCallStateActive) {
      document.getElementById("websbc-ringback").play();
      // make call
      var called_num = $(".webphone-digits").val();
      ...
      document.querySelector(".webphone-digits").value = "";
      if (isFromSSO) {
        patchStatus(callStatus);
      }
      _fSansayUiCallStateHandling(_vSansayUiCallStateActive, called_num);
    } else {
      // end call
      document.getElementById("websbc-ringback").pause();
      _vSansayUiMute = 0;
      _vSansayUiHold = 0;
      _vSansayUiXfer = 0;
  ...
  function _fSansayUiAnswerPrompt(cb, caller, ctype) {
    let callSound = document.getElementById("websbc-ring");
  ...
  $.fn.webphone.login = function (
    ...
    function __incoming_call(caller, session_id, ctype, cb) {
      $(".webphone-digits").val(caller);
      $("#webphone-keypad").addClass("hidden");
      _fSansayUiAnswerPrompt(
        function (ans, caller, ctype) {
          if (ans) {
            console.log(ans, "anssssssssssssssssssssss");
    ...
    _oSansayClient.login({
      ...
      auto_answer_ui_handler: function (remote_user, ctype) {
        ...
        $(".webphone-digits").val(remote_user);
