...
function unsetAdditionalStatus(){
  if(isFromSSO){
    patchStatus({
      additional_status: {
        status: null,
...
function toggleActivePhoneButtons(){
    $("#calling-state").toggleClass("hidden");
    $(".webphone-elapse-time").toggleClass('hidden')
    $("#call-active").toggleClass('hidden')
    $("#call-mute").toggleClass('hidden')
    $("#switch-keypad").toggleClass('hidden')
...
//    self.sm_timer = setTimeout(function(){self.state_table[self.call_state]("E_TIMEOUT", null)}, 1000 * sec);
  ...
  this.startRing = function() {
    ...
    if (typeof self.start_ring === "function")
      ...
      document.getElementById(self.ring_dom_id).muted = false;
    ...
    switch(event) {
      case "E_USER_MAKE_CALL":
        self.mediaEngine.open(
          ...
          self.userHangup, // self.userHangup, need to fix this
          self.processSDP);
